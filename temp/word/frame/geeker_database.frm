...
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} UserForm1 
   ...
   ClientTop       =   456
   ClientWidth     =   13872
   OleObjectBlob   =   "geeker_database.frx":0000
...
Dim db_order As DAO.Database
   ...
   Set rst_order = db_order.OpenRecordset("Select ID , SHEIKH_NAME , DEGREE from  [ORDER] WHERE STATE = 'DESIGN' ORDER BY DEGREE")
  
           
      Do Until rst_order.EOF
          ComboBox1.AddItem (rst_order.Fields("ID") & "-" & rst_order.Fields("SHEIKH_NAME"))
        rst_order.MoveNext
       Loop
...
Private Sub ComboBox1_Change()
   ...
   Set rst_order = db_order.OpenRecordset("Select CONTENT.ID , CONTENT.STUDENT_NAME , CONTENT.QERAA from  CONTENT , [ORDER] WHERE  CONTENT.ORDER_ID = [ORDER].ID AND TYPE = 'EJAZA' AND [ORDER].SHEIKH_NAME = '" & Split(ComboBox1.Value, "-")(1) & "'")
  
           ListBox1.Clear
           
      Do Until rst_order.EOF
          ListBox1.AddItem (rst_order.Fields("ID") & "-" & rst_order.Fields("STUDENT_NAME") & "-" & rst_order.Fields("QERAA"))
        rst_order.MoveNext
       Loop
...
Private Sub CommandButton5_Click()
   ...
   Set rst_order = db_order.OpenRecordset("Select ID from CONTENT where CONTENT.ORDER_ID = " & general_id)
...
Dim FileName As String
    ActiveDocument.ExportAsFixedFormat OutputFileName:= _
        ActiveDocument.path + Application.PathSeparator + "smpl-" + FileName + ".pdf", ExportFormat:=wdExportFormatPDF, _
