...
Dim db_order As DAO.Database
   Dim rst_order As DAO.Recordset
       
   Set db_order = OpenDatabase("E:\\other\\otor.accdb")
     
   Set rst_order = db_order.OpenRecordset("Select SHEIKH.ID , SHEIKH.NAME from SHEIKH , [ORDER] WHERE SHEIKH.NAME = ORDER.SHEIKH_NAME ")
  
           
      Do Until rst_order.EOF
          ComboBox1.AddItem (rst_order.Fields("ID") & "-" & rst_order.Fields("NAME"))
        rst_order.MoveNext
       Loop
   
    db_order.Close
    rst_order.Close
   Set db_order = Nothing
    Set rst_order = Nothing
...
Private Sub ComboBox1_Change()
   ...
   Set rst_order = db_order.OpenRecordset("Select EJAZA.ID , STUDENT_NAME , QERAA from EJAZA , CONTENT , [ORDER] WHERE  EJAZA.CONTENT_ID = CONTENT.ID AND CONTENT.ORDER_ID = [ORDER].ID AND [ORDER].SHEIKH_NAME = '" & Split(ComboBox1.Value, "-") & "'")
  
           
      Do Until rst_order.EOF
          ComboBox1.AddItem (rst_order.Fields("ID") & "-" & rst_order.Fields("STUDENT_NAME") & "-" & rst_order.Fields("QERAA"))
        rst_order.MoveNext
       Loop
...
Private Sub CommandButton5_Click()
   ...
   'make application arabic
   Options.ArabicNumeral = wdNumeralHindi

   Dim general_id As Integer
   general_id = Int(TextBox5.text)
   
   If general_id < 0 Then
    DB_ejaza_creator (-1 * general_id)
    GoTo endL:
   End If
   
   Dim db_order As DAO.Database
   Dim rst_order As DAO.Recordset
   Dim ejazat_id As Integer
       
   Set db_order = OpenDatabase("E:\\other\\otor.accdb")
     
   Set rst_order = db_order.OpenRecordset("Select EJAZA.ID from EJAZA , CONTENT where CONTENT.ID = EJAZA.CONTENT_ID and CONTENT.ORDER_ID = " & general_id)
  
    If rst_order.RecordCount = 0 Then
        TextBox5.text = "„⁄—› «·ÿ·» €Ì— „ÊÃÊœ"
        GoTo endL:
    End If
          
      Do Until rst_order.EOF
           
        TextBox5.text = "found: " & rst_order.Fields("ID")
        DB_ejaza_creator (rst_order.Fields("ID"))
...
Private Sub DB_ejaza_creator(ejaza_id As String)
   ...
   Set db = OpenDatabase("E:\\other\\otor.accdb")
   Set rst = db.OpenRecordset("Select * from EJAZA where ID = " & ejaza_id)
